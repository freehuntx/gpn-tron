...
import { tickrate } from '../shared/constants/common';
...
export class Game extends EventEmitter {
  ...
  constructor(players: Player[]) {
    ...
    this.#width = this.#players.length * 3;
    this.#height = this.#players.length * 3;
  ...
  #onTick() {
    ...
    const winners = this.#players.filter(
      (player) =>
        player.pos.x === this.#maze.goal.x && player.pos.y === this.#maze.goal.y
    );
    if (winners.length > 0) {
      ...
      if (this.#players.length > 1) {
        const losers = this.#players.filter(
          (player) =>
            !(
              player.pos.x === this.#maze.goal.x &&
              player.pos.y === this.#maze.goal.y
            )
        );
        const playersInOrder = [...winners, ...losers];
        const placesInOrder = [
          ...winners.map((player) => 1),
          ...losers.map((player) => 2),
        ];
        const newEloScores = MultiElo.getNewRatings(
          playersInOrder.map((player) => player.eloScore),
          placesInOrder
        );
